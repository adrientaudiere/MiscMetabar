...
#' @param verbose (default = FALSE): whether the function print some information during the computation
...
#' @example
#'data(GlobalPatterns)
#'plot_edgeR_phyloseq(GlobalPatterns, c('SampleType', 'Soil', 'Feces'), color_tax = 'Kingdom')
#'plot_edgeR_phyloseq(GlobalPatterns, c('SampleType', 'Soil', 'Feces'), taxa = 'Class', color_tax = 'Kingdom')
...
plot_edgeR_phyloseq <-
  function(physeq,
    ...
    p <-
      ggplot(sigtabgen, aes(x = tax, y = logFC, color = col_tax), ...) + geom_point(size = 6) +
...
phyloseq_to_deseq2 = function(physeq, design, ...) {
  ...
  if (is.null(sample_data(physeq, FALSE))) {
    stop(
      "There must be sample_data present, for specifying experimental design. See ?phyloseq_to_deseq2"
    )
  }
...
#' @param data (required): a \code{\link{phyloseq-class}} or a \code{\link[DESeq2]{DESeqDataSet-class}} object.
#' @param tax_table : Required if data is a \code{\link[DESeq2]{DESeqDataSet-class}} object.
...
plot_deseq2_phyloseq <-
  function(data,
      ...
      if (!is.null(taxDepth)) {
        ...
        data_TAX@otu_table <-
          otu_table(apply(data@otu_table, 2, function(x)
            tapply(x,
                   data@tax_table[, taxDepth], sum)),
            taxa_are_rows = T)
        data_TAX@tax_table <-
          tax_table(apply(data@tax_table[, 1:match(taxDepth, colnames(data@tax_table))],
                          2, function(x)
                            X <- tapply(x, data@tax_table[, taxDepth], function(xx)
                              xx[1])))
    ...
    if (!sum(areColors(color_tax)) > 0) {
      p <-
        ggplot(d, aes(x = tax, y = log2FoldChange, color = col_tax), ...) + geom_point(size = 6) +
        theme(axis.text.x = element_text(
          angle = -90,
          hjust = 0,
          vjust = 0.5
      ...
      p <-
        ggplot(d, aes(x = tax, y = log2FoldChange), ...) + geom_point(size = 6, color = d$col_tax) +
        theme(axis.text.x = element_text(
          angle = -90,
          hjust = 0,
          vjust = 0.5
