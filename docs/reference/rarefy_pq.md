...
that can repeat the rarefaction `n` times and return the averaged OTU
table, reducing the stochasticity of a single subsampling pass. With
`n = 1` (default) the behaviour matches a standard single rarefaction.

## Usage

``` r
rarefy_pq(physeq, sample_size = NULL, n = 1, seed = 123, ...)
```
...
- seed:
  ...
  (integer, default 123) random seed passed to
  [`phyloseq::rarefy_even_depth()`](https://rdrr.io/pkg/phyloseq/man/rarefy_even_depth.html).
