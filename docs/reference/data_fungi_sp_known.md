...
It is a subset of the data_fungi dataset including only taxa with
...
A physeq object containing 651 taxa with references sequences described
by 14 taxonomic ranks and 185 samples described by 7 sample variables:
...
Obtain using
`data_fungi_sp_known <- subset_taxa(data_fungi, !is.na(data_fungi@tax_table[,"Species"]))`
