...
GlobalPatterns@sam_data[, "Soil_logical"] <- ifelse(
...
test_that("Test one case for each deprecated functions", {
  res_deseq <- DESeq2::DESeq(
    phyloseq_to_deseq2(GP, ~SampleType),
    test = "Wald",
    fitType = "local"
  )
  expect_warning(
    physeq_graph_test(data_fungi_mini, fact = "Tree_name"),
    "deprecated"
  )
  expect_s3_class(
    suppressWarnings(adonis_phyloseq(data_fungi_mini, "Tree_name")),
    "anova"
  )
  ...
  expect_message(expect_warning(
    biplot_physeq(data_fungi_2trees, merge_sample_by = "Tree_name"),
    "deprecated"
  ))
  ...
  expect_warning(
    write_phyloseq(enterotype, path = testFolder, silent = TRUE),
    "deprecated"
  )
  expect_s4_class(
    suppressWarnings(read_phyloseq(testFolder, taxa_are_rows = TRUE)),
    "phyloseq"
  )

  expect_warning(sankey_phyloseq(data_fungi_mini), "deprecated")
  expect_s3_class(
    suppressWarnings(summary_plot_phyloseq(data_fungi_mini)),
    "ggplot"
  )
  expect_message(
    expect_warning(
  ...
  expect_silent(suppressMessages(expect_warning(
    hill_phyloseq(GP, "SampleType"),
    "deprecated"
  )))
  ...
  expect_s3_class(
    suppressWarnings(multiple_share_bisamples(
      data_fungi_low_high,
      bifactor = "Height",
...
suppressWarnings(
  vsearch_error_or_not <- try(
    system("vsearch 2>&1", intern = TRUE),
    silent = TRUE
  )
)
if (class(vsearch_error_or_not) == "try-error") {
  ...
  test_that("lulu_phyloseq works fine", {
    expect_s4_class(
      suppressWarnings(lulu_phyloseq(data_fungi_sp_known)$new_physeq),
      "phyloseq"
    )
  })
}
