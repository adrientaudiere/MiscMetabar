...
test_that(" compare_pairs_pq function works fine with data_fungi dataset", {
  ...
  expect_s3_class(compare_pairs_pq(data_fungi_low_high, bifactor = "Height", merge_sample_by = "Height", vegIndex = "simpson"), "tbl_df")
  ...
  expect_equal(suppressWarnings(sum(is.na(compare_pairs_pq(data_fungi_low_high, bifactor = "Height")))), 9)
})
