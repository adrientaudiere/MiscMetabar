...
if (class(krona_error_or_not) == "try-error") {
  ...
  test_that("krona send an error when krona is not installed", {
    ...
    expect_warning(merge_krona(c(
      "Number.of.sequences.html",
      "Number.of.ASVs.html"
    )))
  })
} else {
  test_that("krona function works fine with GlobalPatterns dataset", {
    ...
    suppressWarnings(unlink(
      list.files(testFolder, full.names = TRUE),
      recursive = TRUE
    ))
    expect_message(krona(
      GA,
      file = paste0(testFolder, "/Number.of.sequences.html")
    ))
    expect_message(krona(
      GA,
      file = paste0(testFolder, "/Number.of.ASVs.html"),
      nb_seq = FALSE
    ))
