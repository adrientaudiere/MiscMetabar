...
* `plot_volcano_pq()` draws a volcano plot (log2 fold change versus -log10 adjusted p-value) from a differential abundance result table such as `DESeq2`, `ancombc_pq()` or `aldex_pq()` output, coloring taxa as up, down or not differentially abundant. The `fc` and `padj` arguments now default to `NULL` and are auto-detected from the input type: the raw list from `ancombc_pq()` is unwrapped to `$res` and its `lfc_*`/`q_*` columns are detected automatically; `aldex_pq()` output is detected via `effect`/`wi.eBH` columns; `lefser_pq()` output (`lefser_df`) triggers a score plot with `|LDA score|` on the y-axis since LEfSe does not produce per-feature p-values.
